...
This `games/rpg/` folder is the classic page-navigation build. The separate graphical map-first build lives in `games/rpg-overworld/`.
...
1. `New Game` creates a default state with Rowan in Thornwake and starts `prologue-arrival`.
2. Story scenes drive dialogue, choices, and scripted effects.
3. Effects can unlock quests, travel, items, journal entries, encounters, and future scenes.
4. Combat rewards feed inventory, XP, silver, and quest progress.
5. Autosave writes the live journey to localStorage while manual slots snapshot the current state.
...
The project is published as static files under `games/rpg/`. Hash routing avoids direct-refresh path issues on static hosts while still satisfying the React Router requirement.
