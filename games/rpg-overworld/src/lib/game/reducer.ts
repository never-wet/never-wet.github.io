...
const runLocationAction = (state: GameState, actionId: string) => {
  ...
  if (action.messageTitle || action.messageBody?.length) {
    openOverworldMessage(state, action.messageTitle ?? action.label, action.messageBody ?? [action.description]);
  }
...
const moveOverworld = (state: GameState, direction: Direction) => {
  ...
  if (steppedInteraction?.trigger === "step") {
    runOverworldInteraction(state, steppedInteraction);
    return;
  }
